...
class DQNLSTM(tf.keras.Model):
    def __init__(self, qth:float,model_name=None, epochs=100,data_config= None,learning_rate=0.001,force_retrain: bool= True,lstm_units: int = 32):
        super().__init__()
        self._input_shape = (data_config["batch_size"], data_config["input_size"], 1)
        self._output_shape = (data_config["output_size"], 1)
        self.memory = []
    ...
    def _build_model(self):
        model = Sequential()
        model.add(LSTM(self.lstm_units, input_shape=(self._input_shape[1], self._input_shape[2]),return_sequences= False))
