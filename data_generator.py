...
class FrequencyResponseGenerator(tf.keras.utils.Sequence):
    def __init__(self, taps: int, padding: int = 100, phase_shift: float = 0.1, resources: int = 1, epoch_size: int = 1000, input_size: int = 100, output_size: int = 10):
        self.signal_length = padding + taps

        assert(self.signal_length >= resources)
    ...
    def __get_slice(self):
        return self.frequency_domain[0, 0 : self.resources * int(self.signal_length / self.resources) : int(self.signal_length / self.resources)]

    def get_data(self, index):
        reshape_vector = [self.resources, 1, 1]
...
class OutageData(FrequencyResponseGenerator):
    def __init__(self, rate_threshold: float, taps: int, padding: int = 100, phase_shift: float = 0.1, resources: int = 1, epoch_size: int = 1000, input_size: int = 100, output_size: int = 10):
        self.rate_threshold = rate_threshold
        super().__init__(taps, padding, phase_shift, resources, epoch_size, input_size, output_size)
    def __getitem__(self, index):
        ...
        reshape_vector = [self.resources, 1]
        y = tf.reshape(y, reshape_vector)
...
if __name__ == "__main__":
    fre_response = FrequencyResponseGenerator(taps = 25, padding=100, resources=2, phase_shift = 0.1)
    fre_response.run_animation(10)
    print(fre_response.padded_signal)

    outage_data = OutageData(taps = 25, padding=10, resources=3, phase_shift = 0.1, rate_threshold = 1.05)
